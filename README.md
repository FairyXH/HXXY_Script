...
华厦学院系统增强工具是一款面向脚本猫、Tampermonkey 等用户脚本管理器的浏览器脚本，运行于华厦学院相关系统页面。脚本提供常用学工操作入口，并集成请求调试、重写规则和接口日志等高级工具。
...
2. 登录[华厦学院学工系统](https://me.hxxy.edu.cn/)。
3. 页面右侧出现“华厦”悬浮按钮后，点击按钮打开增强工具面板。
4. 普通功能位于“工具箱”；请求日志、API 调试、重写规则和开关项可从面板顶部直接进入。

## 完整安装教程

以下内容完整整理自《学工增强脚本安装教程》，保留了原教程的电脑端、手机及平板端步骤和全部配图。

**华厦系统增强脚本使用教程(By Zhang)**

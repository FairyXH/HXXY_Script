...
- **宿舍预选**：帮助新生快速选宿舍(由于微华夏无法通过普通手段在浏览器中打开，请抓包抓取Cookies之后，配合浏览器UA修改插件改为微信UA，然后写入Cookies来访问和加载脚本。或使用Xposed模块注入企业微信。或参考我的另一个项目 [微华厦SDK](https://github.com/FairyXH/MicroHX) ，仅需抓取Cookies在Python中就能完成抢宿舍。)
- **自动任务**：支持间隔时间循环调用销假、学生活动签到签退、假期登记、晚寝签到。
